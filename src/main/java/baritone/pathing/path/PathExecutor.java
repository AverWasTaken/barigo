...
public class PathExecutor implements IPathExecutor, Helper {
    ...
    // keep the launch direction, not the direction of whichever waypoint we cross while airborne
    private final SprintJumpFlight sprintJump;
    ...
    public PathExecutor(PathingBehavior behavior, IPath path) {
        ...
        this.path = path;
        this.pathPosition = 0;
        this.sprintJump = new SprintJumpFlight(behavior.baritone.getInputOverrideHandler()::setInputForceState,
                rotation -> behavior.baritone.getLookBehavior().updateTarget(rotation, false), behavior::sprintJumpAlong);
    }
    ...
    public boolean onTick() {
        ...
        boolean canCancel = movement.safeToCancel() && (!sprintJump.active() || ctx.player().isOnGround());
        ...
        return canCancel && !sprintJump.active(); // don't pause or splice a hop between its waypoints
    }
    ...
    private boolean startGroundShortcut(Movement movement, BlockStateInterface bsi) {
        if (!Baritone.settings().allowGroundShortcuts.value || !canSteerOnGround(movement)
                ...
                || ctx.player().getBbHeight() > 1.8F || findSprintJump(movement) != null) {
            return false;
        }
    ...
    private boolean shouldSprintNextTick() {
        ...
        boolean controlAllowed = !ctx.player().isInWater() && !ctx.player().onClimbable()
                && !ctx.player().horizontalCollision && !((Movement) current).isTargetingBlock() && !hasInteractionInput();
        if (sprintJump.continueFlight(!ctx.player().isOnGround(), controlAllowed, ctx.playerRotations().getPitch())) {
            return true;
        }
        behavior.clearSprintJumpYaw();

        // every landing gets the same checks as the first launch, before ordinary sprint policy.
        if (trySprintJump(current)) {
            return true;
        }

        // traverse requests sprinting, so we need to do this check first
        if (current instanceof MovementTraverse && ctx.player().isOnGround() && !hasInteractionInput()
            ...
            if (next instanceof MovementAscend && sprintableAscend(ctx, (MovementTraverse) current, (MovementAscend) next, path.movements().get(pathPosition + 2))) {
                if (skipNow(ctx, current)) {
                    ...
                    sprintJump.start(next.getDirection(), ctx.playerRotations().getPitch());
                    return true;
                } else {
    ...
    private boolean trySprintJump(IMovement current) {
        Vec3 velocity = ctx.player().getDeltaMovement();
        if (Math.hypot(velocity.x, velocity.z) < 0.08) {
            return false; // reserve the runway while walking up to speed, then take off
        }
        SprintJumpPlan plan = findSprintJump(current);
        if (plan == null) {
            return false;
        }
        sprintJump.start(plan.direction, ctx.playerRotations().getPitch());
        return true;
    }

    private SprintJumpPlan findSprintJump(IMovement current) {
        if (!ctx.player().isOnGround() || ctx.player().isInWater() || ctx.player().onClimbable()
                || ctx.player().horizontalCollision || ctx.player().isCrouching() || ctx.player().isPassenger()
                || ctx.player().getAbilities().flying || ctx.player().getBbWidth() > 0.6F || ctx.player().getBbHeight() > 1.8F
                || !Baritone.settings().allowSprint.value || ctx.player().getFoodData().getFoodLevel() <= 6
                || !Baritone.settings().overshootTraverse.value || hasInteractionInput()
                || ((Movement) current).isTargetingBlock()
                || !behavior.baritone.getInputOverrideHandler().isInputForcedDown(Input.MOVE_FORWARD)) {
            return null;
        }
        // these change flight time/height; the normal movements still work, but our runway estimate doesn't.
        if (ctx.player().hasEffect(MobEffects.JUMP) || ctx.player().hasEffect(MobEffects.SLOW_FALLING)
                || ctx.player().hasEffect(MobEffects.LEVITATION)) {
            return null;
        }
        boolean diagonal = current instanceof MovementDiagonal && current.getDirection().getY() == 0;
        boolean descend = current instanceof MovementDescend && current.getDirection().getY() == -1;
        if (!(current instanceof MovementTraverse || diagonal || descend)) {
            return null;
        }
        if (descend && ((MovementDescend) current).safeMode()) {
            return null;
        }
        BlockStateInterface bsi = behavior.baritone.bsi;
        BlockPos ceilingPos = ctx.playerFeet().above(2);
        boolean ceiling = bsi.get0(ceilingPos).isCollisionShapeFullBlock(bsi.access, ceilingPos);
        if (diagonal && !Baritone.settings().sprintJumpOnDiagonals.value
                || descend && !Baritone.settings().sprintJumpDescend.value
                || ceiling && !Baritone.settings().headHitters.value
                || !ceiling && current instanceof MovementTraverse && !Baritone.settings().sprintJump.value) {
            return null;
        }
        Rotation desired = SprintJumpFlight.rotation(current.getDirection(), ctx.playerRotations().getPitch());
        Rotation actual = behavior.baritone.getLookBehavior().getAimProcessor().peekRotation(desired);
        if (Math.abs(Rotation.normalizeYaw(actual.getYaw() - desired.getYaw())) > 5) {
            return null; // let ground steering align before committing the sprint boost
        }
        double acceleration = ctx.player().getAttributeValue(Attributes.MOVEMENT_SPEED);
        if (!ctx.player().isSprinting()) {
            acceleration *= 1.3;
        }
        boolean downhill = Baritone.settings().sprintJumpDescend.value && !diagonal;
        return SprintJumpPlan.find(path.positions(), pathPosition, ctx.player().position(),
                ctx.player().getDeltaMovement(), acceleration, ceiling, downhill, i -> {
                    Movement next = (Movement) path.movements().get(i);
                    if (!(next instanceof MovementTraverse || next instanceof MovementDiagonal
                            || downhill && next instanceof MovementDescend)) {
                        return false;
                    }
                    next.resetBlockCache();
                    return next.toBreak(bsi).isEmpty() && next.toPlace(bsi).isEmpty() && next.toWalkInto(bsi).isEmpty();
                }, (pos, topY) -> clearJumpColumn(bsi, pos, topY, ceiling));
    }

    private boolean clearJumpColumn(BlockStateInterface bsi, BlockPos pos, int topY, boolean ceiling) {
        if (!clearSmoothingColumn(bsi, pos) || bsi.get0(pos.below()).getBlock().getFriction() != 0.6F) {
            return false;
        }
        for (int y = pos.getY() + 2; y <= topY; y++) {
            BlockPos body = new BlockPos(pos.getX(), y, pos.getZ());
            BlockState state = bsi.get0(body);
            if (MovementHelper.avoidWalkingInto(state) || MovementHelper.isClimbable(state.getBlock())
        ...
        BlockPos overhead = pos.above(2);
        BlockState state = bsi.get0(overhead);
        return state.isCollisionShapeFullBlock(bsi.access, overhead) && !MovementHelper.avoidWalkingInto(state);
    }

    private boolean hasInteractionInput() {
        return behavior.baritone.getInputOverrideHandler().isInputForcedDown(Input.SNEAK)
                ...
                || behavior.baritone.getInputOverrideHandler().isInputForcedDown(Input.CLICK_RIGHT);
    }

    private void endSprintJump() {
        sprintJump.clear();
        behavior.clearSprintJumpYaw();
    }
    ...
    public PathExecutor trySplice(PathExecutor next) {
        ...
        return SplicedPath.trySplice(path, next.path, false).map(path -> {
            ...
            ret.sprintJump.copyFrom(sprintJump);
